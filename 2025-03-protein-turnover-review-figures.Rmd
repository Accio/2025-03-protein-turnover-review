...
ksyn_plot <- ggplot(ksyn_response,aes(x=ligand_conc, y=response, 
  ...
  annotate("text", x=1E-4, y=48, vjust=0.5, hjust=0, size=6,
           label=expression("" %up% k[syn]~":" %up% E[max]~"," - pEC[50])) +
...
kdeg_plot <- ggplot(kdeg_response,aes(x=ligand_conc, y=response, 
  ...
  #              arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text",  x=1E-4,, y=36, vjust=0, hjust=0, size=6,
           label=expression("" %up% k[deg]~":" %down% E[max]~"," %down% pEC[50])) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
kdeg_ksyn_response <- rbind(base_response, high_kdeg_ksyn_response, low_kdeg_ksyn_response) %>%
  ...
  mutate(k_deg_syn=interaction(factor(k_deg,
                               ...
                               factor(k_syn,
                                      levels=unique(sort(k_syn, decreasing=TRUE))),
                               sep = "/", drop=TRUE))
kdeg_ksyn_plot <- ggplot(kdeg_ksyn_response,aes(x=ligand_conc, y=response, 
  ...
  #              col="black", inherit.aes = FALSE, 
  #              arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text",  x=1E-4,, y=58, vjust=0, hjust=0, size=6,
           label=expression("" - frac("" %up% k[deg], "" %up% k[syn])~":" %up% E[max]~"," %down% pEC[50])) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
keRL_plot <- ggplot(keRL_response,aes(x=ligand_conc, y=response, 
  ...
  #              col="black", inherit.aes = FALSE, 
  #              arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=1E-4, y=60, vjust=0, hjust=0, size=6,
           label=expression("" %up% k[e(RL)] ~":" %down% E[max]~"," %up% pEC[50])) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
rev_irrev_plot <- ggplot(rev_irrev_response,
  ...
  geom_segment(aes(y=0, yend=0.5,
                   x=EC_50, col=k_eRL_factor), lty=2, size=1) +
  geom_segment(x=-2.5, xend=-3.6,
               y=1.08, col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=1E-3, y=1.1, vjust=0, hjust=0.5,
           label="irreversible", size=6) +
  annotate("text", x=2E-4, y=1.05, vjust=1, hjust=0, size=6,
           label=expression("" %up% frac(k[e(RL)], k[off]) ~":" %up% pEC[50])) +
  geom_line() +
  empty_theme + 
  theme(axis.text.x = element_text(size=15, angle=30, margin=margin(t=22, b=-10))) +
...
top_rows <- plot_grid(add_simp_xlab(ksyn_plot), add_simp_xlab(kdeg_plot),
                      add_simp_xlab(kdeg_ksyn_plot), add_simp_xlab(keRL_plot), label_size=18,
