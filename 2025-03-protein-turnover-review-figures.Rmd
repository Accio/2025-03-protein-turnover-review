...
ksyn_plot <- ggplot(ksyn_response,aes(x=ligand_conc, y=response, col=k_syn_factor)) +
  scale_color_manual(values=syn_col, name=expression(k[syn]),
                     labels=var_ab_labels) +
  geom_line() +
  geom_segment(aes(x=1E-4, xend=EC_50,
                   y=E_0 + 0.5*E_max, col=k_syn_factor), lty=2) +
  geom_segment(aes(y=0, yend=E_0 + 0.5*E_max,
                   x=EC_50, col=k_syn_factor), lty=2) +
  geom_segment(x=-3.5,
               y=40, yend=46,
               col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=2E-3, y=43, vjust=0.5, hjust=0.5, size=6,
           label=expression(k[syn] %up% "")) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
  scale_y_continuous(limits=c(0, max(ksyn_response$response)*1.05),
                     expand=c(0,0)) +
  empty_theme +
  xlab("Ligand concentration [log scale]") + ylab("Response [A.U.]")
print(ksyn_plot)
```
...
kdeg_plot <- ggplot(kdeg_response,aes(x=ligand_conc, y=response, col=k_deg_factor)) +
  scale_color_manual(values=deg_col, name=expression(k[deg]),
                     labels=var_ab_labels) +
  geom_segment(aes(x=1E-4, xend=EC_50,
               y=E_0 + 0.5*E_max, col=k_deg_factor), lty=2) +
  geom_segment(aes(y=0, yend=0.5*E_max+E_0,
                   x=EC_50, col=k_deg_factor), lty=2) +
  geom_segment(x=-3.5, xend=-3,
               y=36, yend=32, col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=6E-3, y=32, vjust=0, hjust=0.5, size=6,
           label=expression(k[deg] %up% "")) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
  scale_y_continuous(limits=c(0, max(kdeg_response$response)*1.05),
                              expand=c(0,0)) +
  geom_line() +
  empty_theme +
  xlab("Ligand concentration [log scale]") + ylab("Response [A.U.]")
...
kdeg_ksyn_plot <- ggplot(kdeg_ksyn_response,aes(x=ligand_conc, y=response, col=k_deg_syn)) +
  ...
  geom_segment(aes(x=1E-4, xend=EC_50,
               y=E_0 + 0.5*E_max, col=k_deg_syn), lty=2) +
  geom_segment(aes(y=0, yend=0.5*E_max+E_0,
                   x=EC_50, col=k_deg_syn), lty=2) +
  geom_segment(x=-3.5, xend=-3.2,
               y=50, yend=57,
               col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=1E-2, y=52, vjust=0.5, hjust=0.5, size=6,
           label=expression(k[deg]~"&"~k[syn] %up% "")) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
  scale_y_continuous(limits=c(0, max(kdeg_ksyn_response$response)*1.05),
                              expand=c(0,0)) +
  empty_theme +
  xlab("Ligand concentration [log scale]") + ylab("Response [A.U.]")
...
keRL_plot <- ggplot(keRL_response,aes(x=ligand_conc, y=response, col=k_eRL_factor)) +
  geom_line() +
  scale_color_manual(values=eRL_col, 
                     name=expression(k[e(RL)]),
                     labels=var_ab_labels) +
  geom_segment(aes(x=1E-4, xend=EC_50,
               y=E_0 + 0.5*E_max, col=k_eRL_factor), lty=2) +
  geom_segment(aes(y=0, yend=0.5*E_max+E_0,
                   x=EC_50, col=k_eRL_factor), lty=2) +
  geom_segment(x=-3.2, xend=-3.5,
               y=57, yend=50,
               col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=1E-3, y=50, vjust=0.5, hjust=0, size=6,
           label=expression(k[e(RL)] %up% "")) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
  scale_y_continuous(limits=c(0, max(keRL_response$response)*1.05),
                              expand=c(0,0)) +
  empty_theme +
  xlab("Ligand concentration [log scale]") + ylab("Response [A.U.]")
print(keRL_plot)
...
```{r rev_irrev_plot, fig.height=4, fig.width=6}
...
rev_irrev_plot <- ggplot(rev_irrev_response,
                         aes(x=ligand_conc, y=(response-E_0)/(E_max), col=k_eRL_factor)) +
  ...
  annotate("text", 
           x=rev_irrev_ec50*c(0.25, 1, 4), y=0.5, vjust=0, size=6, col=rev_irrev_col, 
  ...
  scale_x_log10(limits=c(1E-4,1E1), expand=c(0,0),
                ...
                labels=c(expression(EC[50] %~~% frac(k[deg], k[on])),
                         expression(EC[50] == frac(k[off] + k[deg], k[on])),
                         expression(EC[50] %~~% frac(k[deg], k[e(RL)]) %.% frac(k[off], k[on])))) + 
  scale_y_continuous(limits=c(0, 1), expand=c(0,0), labels=scales::percent) +
  geom_segment(aes(x=1E-4, xend=EC_50,
               y=0.5, col=k_eRL_factor), lty=2) +
  geom_segment(aes(y=0, yend=0.5,
                   x=EC_50, col=k_eRL_factor), lty=2) +
  geom_segment(x=-2.5, xend=-3.5,
               y=0.8, col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=1E-3, y=0.825, vjust=0, hjust=0.5,
           label="irreversible", size=6) +
  geom_line() +
  empty_theme + 
  theme(axis.text.x = element_text(size=12, angle=30, margin=margin(t=18))) +
...
```{r layout_grid_arrange, eval=FALSE}
ksyn_keg_kRL_layout <- matrix(c(1,2,
                                1,2,
                                3,4,
                                3,4,
                                5,5,
...
grid.arrange(ksyn_plot, 
             kdeg_plot,
             kdeg_ksyn_plot, 
             keRL_plot,
             rev_irrev_plot,
             layout_matrix=ksyn_keg_kRL_layout)
```


```{r ksyn_keg_kRL_plots, fig.height=11, fig.width=8}
top_rows <- plot_grid(ksyn_plot, kdeg_plot,
                      kdeg_ksyn_plot, keRL_plot, label_size=18,
                      nrow=2, ncol=2, labels="AUTO")
full_plot <- plot_grid(top_rows,
                       rev_irrev_plot,
                       rel_heights = c(4,3), label_size=18,
                       labels=c("", "E"), ncol=1)
print(full_plot)
```
...
base_ab_ss_plot <- ggplot(base_ab_ss,
  ...
  xlab(expression(paste("Steady-state ligand concentration ",
                        L[ss], " in log scale"))) + ylab("Concentration [A.U.]")
