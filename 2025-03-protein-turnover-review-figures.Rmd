...
ksyn_plot <- ggplot(ksyn_response,aes(x=ligand_conc, y=response, 
  ...
  annotate("text", x=1E-4, y=48, vjust=0.5, hjust=0, size=6,
           label=expression("" %up% k[syn]~"⟹" %up% E[max]~"," - EC[50])) +
...
kdeg_plot <- ggplot(kdeg_response,aes(x=ligand_conc, y=response, 
  ...
  annotate("text",  x=1E-4,, y=36, vjust=0, hjust=0, size=6,
           label=expression("" %up% k[deg]~"⟹" %down% E[max]~"," %up% EC[50])) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
kdeg_ksyn_plot <- ggplot(kdeg_ksyn_response,aes(x=ligand_conc, y=response, 
  ...
  annotate("text",  x=1E-4,, y=58, vjust=0, hjust=0, size=6,
           label=expression(paste(" ",
                                  frac("" %up% k[syn], "" %up% k[deg])~"⟹" %up% E[max]~"," %up% EC[50]))) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
keRL_plot <- ggplot(keRL_response,aes(x=ligand_conc, y=response, 
  ...
  annotate("text", x=1E-4, y=60, vjust=0, hjust=0, size=6,
           label=expression("" %up% k[e(RL)] ~"⟹" %down% E[max]~"," %down% EC[50])) +
  scale_x_log10(limits=c(1E-4,1E2), expand=c(0,0)) +
...
rev_irrev_plot <- ggplot(rev_irrev_response,
  ...
  geom_segment(x=-3.1, xend=-3.9,
               ...
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=3.2E-4, y=0.25, vjust=0, hjust=0.5, label="irreversible", size=4.5) +
  geom_segment(x=0.15, xend=0.95,
               y=0.35, col="black", inherit.aes = FALSE, 
               arrow = arrow(length = unit(0.03, "npc"))) +
  annotate("text", x=3.5, y=0.25, vjust=0, hjust=0.5, label="reversible", size=4.5) +
  annotate("text", x=1E-4, y=1.2, vjust=1, hjust=0, size=6,
...
cov_pk_pd_plot_withMargins <- cov_pk_pd_plot +
  theme(plot.margin = margin(l=2, r=2, unit="cm"))
